...
Note that we have neglected the commonly included $\tfrac{1}{n!}$ factor in
the anti-symmetriser operator.
...
anti-symmetriser, defined in \autoref{eq:anti-symmetriser}, to construct a
...
$\ket*{\psi_{\alpha}^{\lr{S}}}$ with total spin $S$ (as is the case for hydrogen
and helium targets), we may instead write
\begin{equation}
  \label{eq:ccc-v-spin}
  \hat{V}^{\lr{S}}
  \hat{I}_{T}^{\lr{N}}
...
wavefunction $\ket*{\Psi_{i}^{\lr{N, S}}}$ can be derived.
...
The $T$ matrix can be reconstructed from the $K$ matrix by the identity
\begin{equation}
  \label{eq:ccc-t-k}
  \mel*
  {\vb{k}_{f} \Phi_{n_{f}}^{\lr{N}}}
