...
considering how to construct the two into a total state.
...
we might naively suppose the total state of the projectile-target system can be
constructed as a tensor products of the individual projectile and target states.
That is, we might suppose that the total asymptotic state,
$\ket*{\Psi^{\lr{+}}}$, may be
written as $\ket*{\Psi^{\lr{+}}} = \ket{\phi, \boldsymbol{k}}$,
where $\ket{\phi}$ is the target state, and $\ket{\boldsymbol{k}}$ is the
projectile state.
However, this neglects to account for the effects of spin and Pauli's exclusion
...
To account for this principle, we may write the total state,
$\ket*{\Psi^{\lr{S+}}}$, for a given total spin $S$, in the form
\begin{equation}
...
$\ket*{\psi^{\lr{S+}}}$ is the two-fermion state.
It follows that the total state, in this form, is guaranteed to adhere to
