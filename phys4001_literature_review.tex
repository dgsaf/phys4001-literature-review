...
The projectile states, $\ket{\vb{k}} \in \mathcal{H}$, are defined to be
...
To ensure that the total wavefunction is anti-symmetric, we construct by
anti-symmetrising a multichannel expansion of the form
\begin{equation}
...
\ker\lr{\hat{A}}$, and constant $\alpha \in \complex$, the multichannel
...
The asymptotic eigenstates taken to be of the form
\begin{equation}
...
are plane waves, defined in \autoref{eq:projectile-states}.
...
anti-symmetrisation operator, and is subject to the constraints imposed in
\autoref{eq:multichannel-constraint} to ensure uniqueness.
We note that with these expressions for the asymptotic eigenstates and the total
wavefunction, that the $\hat{T}$ operator is related to the potential $\hat{V}$
by the expression
\begin{equation}
  \label{eq:ccc-t-v}
  \hat{T}
  ...
  .
\end{equation}
However, in the case where the potential $\hat{V}$ can be re-written in a way
which accounts for the anti-symmetrisation of the un-symmetrised wavefunction
$\ket*{\psi_{\alpha}^{\lr{S}}}$ with total spin $S$, we may write
\begin{equation}
  \label{eq:ccc-v-spin}
  \hat{V}^{\lr{S}}
  \hat{I}_{T}^{\lr{N}}
  ...
  ,
\end{equation}
in which case the $\hat{T}$ operator is instead written as
\begin{equation}
  \label{eq:ccc-t-v-spin}
  \hat{T}^{\lr{S}}
  \ket*{\Phi_{i_{\alpha}}^{\lr{N}} \vb{k}_{\alpha}}
  =
  \hat{V}^{\lr{S}}
  ...
  .
\end{equation}
The Convergent Close-Coupling equations are therefore of the form
\begin{alignat}{2}
  \label{eq:ccc-ls-t}
  &
  \mel*
  {\Phi_{n_{f}}^{\lr{N}} \vb{k}_{f}}
  {\hat{T}^{\lr{S}}}
