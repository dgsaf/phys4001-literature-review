...
two-body system of hydrogen, the CCC method was extended to three-body systems
\cite{PhysRevLett.69.53, BRAY19951, PhysRevLett.89.273201}, including
helium targets \cite{PhysRevA.52.1279, Fursa_1997}, as well as hydrogenic
targets \cite{}.
The CCC method was also extended to include photon \cite{Bray_2002}, and
positron \cite{} projectiles.
...
similar in form to the derivations presented in \cite{AJP_BRAY1996, BRAY19951}.
...
discussed in \autoref{sec:e-h-uniqueness}.
...
attention to the asymptotic eigenstates for which the target pseudostate
$\lrset{\ket*{\Phi_{n}^{\lr{N}}}}_{n = 1}^{N_{T}}$ and projectile both have
$l = 0$.
...
Much of the following treatment is generalisable to a full treatment of
angular momentum.
...
amplitudes $F_{\alpha, \beta}$, not the multichannel weight functions
$\ket*{F_{n}^{\lr{N}}}$ defined in
\autoref{eq:total-wavefunction-un-symmetrised-n}.
...
However it can be shown, as discussed in \cite{BRAY19951}, that the projection
of a continuum wave onto a $N$-dimensional Laguerre basis is conditionally
convergent as $N$ increases, but is numerically stable.
...
\paragraph{Step Function Behaviour of TICS}
\label{sec:e-h-step-function}
