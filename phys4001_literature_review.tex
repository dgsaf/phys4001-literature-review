...
the target orbitals into a core and valence set of orbitals, with the core
orbitals being limited to a much smaller set of states, while the valence
orbitals are not so constrained.
This provides an effective model for targets with a mostly fixed set of electron
states, while allowing the valence electrons to interact fully with the
projectile.
...
then be solved using the CCC formalism.
...
integration over the uncountably infinite states, for which the eigen-energy
...
This can be seen by applying the operator $\lrsq{E - \hat{H}_{\rm{A}}}$ on the
left of \autoref{eq:ls},
\begin{equation}
  \label{eq:ls-kernel}
  \lrsq
  {
...
\begin{equation}
  ...
  ,
\end{equation}
yielding the formulation of the Lippmann-Schwinger equation in terms of
$\hat{T}$.
...
\begin{equation}
  ...
  ,
\end{equation}
where $\delta_{\alpha, \beta}$ appropriately represents the Kronecker delta and
Dirac delta for the asymptotic states, and that these states satisfy
\begin{equation}
  \label{eq:asymptotic-matrix-element}
  \mel*
  {\Omega_{\alpha}}
  {E - \hat{H}_{\rm{A}}}
  {\Omega_{\beta}}
  =
  \lr{E - \varepsilon_{\beta}}
  \braket*
  {\Omega_{\alpha}}
  {\Omega_{\beta}}
  =
  \lr{E - \varepsilon_{\alpha}}
  \delta_{\alpha, \beta}
  .
\end{equation}
From the definition \autoref{eq:greens-operator}, we must have that
\begin{equation}
  \label{eq:greens-operator-matrix-element-1}
  \mel*
  {\Omega_{\alpha}}
  {
    \hat{G}_{\lr{E}}
...
\begin{equation}
  \label{eq:greens-operator-matrix-element-2}
  \mel*
  {\Omega_{\alpha}}
  {\hat{G}_{\lr{E}}}
  {\Omega_{\beta}}
...
\end{equation}
and so the Green's operator can be expanded in the set of asymptotic states to
be written in the form
\begin{equation}
  \label{eq:greens-operator-expansion}
  \hat{G}_{\lr{E, \pm}}
  =
  \lim_{\eta \to 0}
  \sum_{\gamma}\int
  ...
  ,
\end{equation}
where the imaginary limit exists to ensure the integral is well-defined for all
$\varepsilon_{\gamma}$.
With an expression for the Green's operator in hand, we can now re-write the
Lippmann-Schwinger equation in the following form
\begin{equation}
  \label{eq:ls-t}
  \mel*
  {\Omega_{\alpha}}
  ...
  ,
\end{equation}
which conveniently expresses all matrix elements in terms of the asymptotic
eigenstates.
...
asymptotic Hamiltonian and a decaying potential in the form
\begin{equation}
...
\begin{equation}
  ...
  ,
\end{equation}
and where the decaying potential is of the form
\begin{equation}
  ...
  ,
\end{equation}
where $\hat{U}_{0}$ is the asymptotic potential acting on the projectile.
The asymptotic eigenstates are therefore taken to be of the form
\begin{equation}
  \label{eq:ccc-asymptotic-states}
  \ket*{\Omega_{\alpha}}
  =
  \ket*{\Phi_{i_{\alpha}} \vb{k}_{\alpha}}
  \approx
  \ket*{\Phi_{i_{\alpha}}^{\lr{N}} \vb{k}_{\alpha}}
  ,
\end{equation}
with corresponding asymptotic eigenenergies
\begin{equation}
...
pseudostates, defined in \autoref{eq:target-states}, and where $\ket*{\vb{k}}$
are the plane/coulomb/distorted waves, defined to be eigenstates of the
projectile component of the asymptotic Hamiltonian,
\begin{equation}
  \label{eq:ccc-waves}
  \lrsq
  {
...
wavefunction, that the $\hat{T}$ operator is related to the decaying potential
$\hat{W}$ by the expression
\begin{equation}
  \label{eq:ccc-t-w}
  \hat{T}
  \ket*{\Phi_{i_{\alpha}}^{\lr{N}} \vb{k}_{\alpha}}
  ...
  .
\end{equation}
However, we can recast this potential in a way which accounts for the explicit
anti-symmetrisation of the total wavefunction.
...
\begin{equation*}
  ...
  ,
\end{equation*}
with this operator expanding to the form
\begin{equation*}
  \lrsq
  {
    E
  ...
  ,
\end{equation*}
whence we define the explicitly anti-symmetrised potential $\hat{V}$ to be of
the form
\begin{equation}
  \label{eq:ccc-v}
  \hat{V}
  ...
  \hat{P}_{0, m}
\end{equation}
for which we can see that the explicit anti-symmetrisation of
$\ket*{\psi_{\alpha}^{\lr{+}}}$ is handled by $\hat{V}$,
\begin{equation}
  \label{eq:ccc-v-2}
  \hat{H}
  \ket*{\psi_{\alpha}^{\lr{+}}}
  ...
  .
\end{equation}
This allows us to write the $\hat{T}$ operator in the form
\begin{equation}
  \label{eq:ccc-t-v}
  \hat{T}
  \ket*{\Phi_{i_{\alpha}}^{\lr{N}} \vb{k}_{\alpha}}
...
We shall restrict our attention to the case of single ionisation.
...
target state $\ket*{\Phi_{\alpha}^{+}}$ and a continuum wave
$\ket*{\vb{q}_{\alpha}}$; that is,
\begin{equation}
...
Evaluating the ionisation scattering amplitudes at any other energies requires
an interpolation between the energies at which it can be evaluated at.
