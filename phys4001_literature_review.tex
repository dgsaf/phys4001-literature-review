...
$\ket{\Psi}$.
Hence, without loss of generality, we may simplify the generalised sum/integral
to a simpler form by writing
\begin{equation}
  ...
  ,
\end{equation}
where $\ket*{\Omega_{\alpha}} \in \ker\lr{E - \hat{H}_{\rm{A}}}$, for which
$\ket{\Psi_{\alpha}}$ is the eigenstate of $\hat{H}$ corresponding to the
...
\begin{equation}
  ...
  ,
\end{equation}
where the imaginary limit is present to ensure that the integral is well-defined
for all $\varepsilon_{\gamma}$.
...
\begin{equation}
  ...
  ,
\end{equation}
which expresses the representation of the operator $\hat{T}$ in terms of the
asymptotic eigenstates.

As a final note on this general form of the Lippmann-Schwinger equation,
\autoref{eq:ls-t}, it can be seen that the
