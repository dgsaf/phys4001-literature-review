...
triple-differential cross sections (SDCS, DDCS, TDCS) - failed to demonstrate
convergence, discussed in detail by Bray \cite{PhysRevLett.78.4721}.
...
However, issues arise for the triplet case as the SDCS assumes a non-zero value
...
Accounting for this behaviour, Bray \cite{PhysRevLett.89.273201}, was able to
demonstrate the validity of the CCC method by reproducing differential cross
...
and Bartschat to electron-impact ionisation of helium, with and without
...
may exist with its application.
...
\begin{equation}
  ...
  ,
\end{equation}
where $e_{q_{\alpha}} = \tfrac{1}{2} q_{\alpha}^{2}$ is the energy of the outgoing
...
Hence, integrating the DDCS over the spherical coordinates of the
